...
public:
	...
	// 赋值运算符重载
	void Revolute1(BinTreeNode<ElemType> *p); // 递归实现左右子树交换
	void Revolute2(BinTreeNode<ElemType> *p); // 非递归实现左右子树交换
};
template <class ElemType>
void Revolute1(BinTreeNode<ElemType> *root)
// 递归实现二叉树交换左右子树
{
	if (root == NULL)
		return;
	BinTreeNode<ElemType> *temp = root->leftChild;
	root->leftChild = root->rightChild;
	root->rightChild = temp;
	Revolute1(root->leftChild);
	Revolute1(root->rightChild);
}

template <class ElemType>
void Revolute2(BinTreeNode<ElemType> *root)
{
	// 非递归方法实现二叉树交换左右子树（数组模拟队列进行层次遍历）
	if (root == NULL)
		return;
	BinTreeNode<ElemType> *q[MAXSIZE];
	int front = -1;
	int rear = 0;
	q[rear] = root;
	while (rear != front)
	{
		BinTreeNode<ElemType> *p = q[++front];
		// 交换左右子树
		BinTreeNode<ElemType> *temp = p->leftChild;
